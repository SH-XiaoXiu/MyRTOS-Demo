...
一个平台无关的多任务调度系统,已在Cortex-M4/M3移植验证。实现了内核、服务与平台/架构驱动的深度解耦。结构清晰简单、可移植性强，并提供了内核扩展钩子，作为苯人学习实时操作系统原理、研究可移植系统设计的项目。


(截止至2026.1.21,已在QEMU上模拟)
![boot](assets/boot.png)

> **TIP:**
> 开发到当前的状态, 我发现本项目的设计和实现已经逐渐偏离了RTOS的范畴
> 但是此项目依旧可以裁切为单片机的RTOS, 只是上层更多的抽象和实现已经不在适合作为RTOS开发
> 因此结合之前的经验,苯人正在学习并开发一个更完善,更贴合现代的操作系统Demo <https://gitee.com/sh-xiaoxiu/xnix> (可能会咕咕咕)
