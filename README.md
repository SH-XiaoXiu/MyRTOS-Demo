...
* **内存管理**：
    * 基于静态内存池的动态内存分配 (`rtos_malloc`/`rtos_free`)。
    * 线程安全，支持空闲块自动合并以减少碎片。
* **任务管理**：
    * 支持任务的动态创建与删除。
    * 实现任务ID的回收与复用。
...
* **系统监控与调试**：
    ...
    * HardFault异常信息打印。
* **异步日志服务**：通过独立任务处理日志I/O，支持多级别日志输出。
* **系统可配置**：通过头文件 (`MyRTOS_Config.h`) 对内核功能和资源进行配置。
