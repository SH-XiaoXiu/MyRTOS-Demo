/**
 * @brief MyRTOS QEMU MPS2-AN385 Demo
 * @author XiaoXiu
 * @note  基于 GD32 Demo 移植，移除了硬件相关部分（LED、按键中断）
 *
...
static TaskHandle_t g_high_prio_task_h, g_background_task_h;
...
static void on_services_init(void) {
    ...
    if (console) {
        ...
        Stream_Printf(console, "  CPU: ARM Cortex-M3 (QEMU MPS2-AN385)\r\n");
        Stream_Printf(console, "  Clock: %lu MHz\r\n", MYRTOS_CPU_CLOCK_HZ / 1000000);
        Stream_Printf(console, "  Console: UART0 @ 115200 baud\r\n");
#if PLATFORM_USE_HIRES_TIMER == 1
        Stream_Printf(console, "  Timer: TIMER%d (high-resolution counter)\r\n", PLATFORM_HIRES_TIMER_NUM);
#endif
...
static void create_tasks(void) {
    ...
    if (g_recursive_lock) {
        g_recursive_task_h = Task_Create(recursive_test_task, "RecursiveTask", 256, NULL, COLLABORATION_TASKS_PRIO);
    }

    // 创建其他演示任务
    g_a_task_h = Task_Create(a_task, "TaskA", 256, NULL, COLLABORATION_TASKS_PRIO);
    g_b_task_h = Task_Create(b_task, "TaskB", 256, NULL, COLLABORATION_TASKS_PRIO);
    g_d_task_h = Task_Create(d_task, "TaskD_Creator", 256, NULL, COLLABORATION_TASKS_PRIO);
    g_background_task_h = Task_Create(background_task, "BG_Heartbeat", 256, NULL, BACKGROUND_TASK_PRIO);
...
static int blinky_main(int argc, char *argv[]) {
    ...
    int count = 0;
    while (1) {
        MyRTOS_printf("[blinky] Heartbeat %d\n", count++);
        ...
        if (signals & SIG_INTERRUPT) {
            MyRTOS_printf("[blinky] Received interrupt, exiting...\n");
            return 0;
        }
    }
